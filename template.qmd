...
To give a long caption, the easiest thing to do is to add a chunk like this right before it.
The upstream LaTeX template uses `floatnote`s for the long caption and `fig-cap` for the title.
`floatnote`s latch onto the next float, so they should be placed right before the float they are meant to describe.
